...
#endif
    if (num == SYS_execve) {
        fprintf(trace,"0x%lx: strace(%s, [",
            (unsigned long)ip,
            (char *)arg0
        );
        for (int i = 0; (char *)(arg0 + sizeof(ADDRINT) * i) != NULL; i += 1) {
            fprintf(trace,"\"%s\",",
                (char *)(arg0 + sizeof(ADDRINT) * i)
            );
        }
        fprintf(trace,"], ...)\n");
    }
    if (num == SYS_write) {
        fprintf(trace,"0x%lx: write(%lx, \"%s",
            (unsigned long)ip,
            (unsigned long)arg0,
            (char *)arg1
        );
        // write(fileno(trace), (char *) arg1, arg2);
        fprintf(trace,"\", %lx)\n",
            (unsigned long)arg2
        );
    }
